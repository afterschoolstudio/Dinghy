...
internal static class Program
{
    ...
    static util.NativeArray<sgp_vec2> ptbf = new util.NativeArray<sgp_vec2>(4096);
    static byte[] testImgBytes;
    static sg_image image;
    private static void Main()
    {
        for (int i = 0; i < 4096; i++)
        {
            ptbf[i] = default(sgp_vec2);
            // points_buffer[i] = default(sgp_vec2);
        }
        // bufferHandle = GCHandle.Alloc(points_buffer,GCHandleType.Pinned);

        testImgBytes = File.ReadAllBytes("testimg.png");
    ...
    private static unsafe void Initialize()
    {
        ...
        fixed (byte* imgPtr = testImgBytes)
        {
            ...
            image = Sokol.sg_make_image(&image_desc);
